...
use bevy::prelude::*;
use component::{ComponentBuilder, EventHandlerInstaller};
use dioxus_core::{
    AttributeValue, Element, ScopeId, TemplateAttribute, TemplateNode, VirtualDom, WriteMutations,
};
...
pub struct DioxusRoot {
    pub root: fn() -> Element,
    pub element_map: HashMap<dioxus_core::ElementId, Entity>,
}
...
impl WorldRenderer {
    ...
    fn command(&mut self, command: impl FnOnce(&mut World) + 'static) {
        self.commands.push(Box::new(command));
    }

    fn drain_commands(&mut self) -> Vec<Box<dyn FnOnce(&mut World) + 'static>> {
